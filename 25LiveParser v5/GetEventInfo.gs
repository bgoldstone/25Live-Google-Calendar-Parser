/*
  Calls 25Live API to get more Event Details.
 */
function getRequirements(eventID) {
  ...
  let eventCategory = ''
  try{
   description = `${json['evdetail']['defn']['panel'][1]['item'][0]['itemName']}`.replace("<br />","\n").replace(tags,"\n").replaceAll("\n","\n\t")
   ...
   try{
   comments = json['evdetail']['defn']['panel'][2]['item'][0]['item'][0]['itemName'].replaceAll("\n","\n\t")
   }catch{
     comments = noDetails
   }
   try{
   confirmationText = json['evdetail']['defn']['panel'][4]['item'][0]['itemName'].replaceAll("\n","\n\t")
   }catch{
     confirmationText = noDetails
   }
   try{
   eventCategory = json['evdetail']['defn']['panel'][0]['item'][10]['item'][0]['itemName']
   }catch{
     eventCategory = undefined
   }
   try{
   expectedHeadCount = json['evdetail']['defn']['panel'][0]['item'][11]['item'][0]['item'][0]['itemLabel']
   } catch{
     expectedHeadCount = undefined
   }
   try{
   registeredHeadCount = json['evdetail']['defn']['panel'][0]['item'][11]['item'][0]['item'][1]['itemLabel']
   }catch{
     registeredHeadCount = undefined
   }
  for(let requirement in requirements){
    for(let item in requirements[requirement]){
    returnArray.push(`${requirements[requirement][item]["itemLabel"]} : ${requirements[requirement][item]["itemName"] ? requirements[requirement][item]["itemName"] : noDetails}`)
    }
  }
  ...
  let headCount = `\n\nHead Count:\n\t${expectedHeadCount ? expectedHeadCount : 0} expected\n\t${registeredHeadCount ? registeredHeadCount : 0} registered`
